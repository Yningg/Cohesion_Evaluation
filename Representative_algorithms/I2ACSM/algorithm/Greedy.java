...
public class Greedy {
	...
	public void PrecRec(BigInteger[] so,Graph G, Graph G0){//precision、recall和F1,G为初始化图，G0为得到的结果图
		int p = 0;
		int len = 0;
...
//		return score;//这里实验无影响力
	...
	public boolean isContains(List<Vertex> VertexList, BigInteger vId)//判断一个list中是否有ID为vId的节点
	{
	...
	public Graph greedyWeight(Graph H,Graph G,int k,int d,Vertex q)//贪心函数,参数H为社区,参数G为原始图
	{
		...
		for(int a=0;a<H.getVertexIdList().size();a++)//给H0的VertexList赋初值
		{
			...
			for(BigInteger in : H.getVertexIdList().get(a).getNeighborWeight().keySet())//权重赋值
			{
				v.getNeighborWeight().put(in, H.getVertexIdList().get(a).getNeighborWeight().get(in));
			}
			H0.getVertexIdList().add(v);
		}
		for (BigInteger in : H.getVertexMap().keySet()) 
		{
			...
			for(BigInteger xy: H.getVertexMap().get(in).getNeighborWeight().keySet())//权重赋值
			{
		...
		while(H.getVertexMap().size()>k)
		{
			...
			else
			{
				...
				{
					System.out.println("Delete the query node");
					break;
				}
