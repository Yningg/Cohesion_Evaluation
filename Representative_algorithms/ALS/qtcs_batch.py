...
class Graph:
    def __init__(self, dataset):
        # Load the dataset from ./data folder and build the graph
        ...
        print("self.tmax:" + str(self.tmax)) # maximum number of interactions between two nodes
    ...
    def TemporalGraph(self):
        ...
        with open(self.dataset, 'r') as file:
            ...
            for line in lines:
                ...
                try:
                    ...
                    continue
                if from_id == to_id: # Exclude self-loop
                    continue
                for (f_id, t_id) in [(from_id, to_id), (to_id, from_id)]:
                    temp.add((f_id, t_id, time_id))
        temp = list(temp)
        temp.sort(key=lambda x: x[2]) # Sort by time

        # Construct edge stream, where timestamp might be the same, t_index is the time index for recording the unique timestamp
        edge_stream = [(temp[0][0], temp[0][1], 1)] # Index start from 1: means transfer the real timestamps to the new index start from 1, also neglecting the real time interval 
        ...
        # Construct the temporal adjacency dictionary (directed)
    ...
    # Calculate the temporal transition probability (TTP) and dangling state
    def Ttp(self):
        starttime = time.time()
    ...
    # Compute Time-Constrained Personalized PageRank (TPPR) for the vertex
    ...
    # The EGR Algorithm: calculate the TPPR of each vertex and then greedily remove the vertices with the minimum query-biased temporal degree.
    def EGR(self, alpha, seed):
        ...
        result = self.maintain_connected(R, seed)
        endtime = time.time()
        return result, time_tppr, endtime - starttime, opt
    ...
    def ALS(self, alpha, seed):
        ...
        return C, expanding_time, reducing_time, result, lambda_1, best
...
def process_query_node(query_node, G_qtcs, alpha):
    ...
    print("query_node: " + str(query_node))

    result, time_tppr, t_egr, beta = G_qtcs.EGR(alpha, query_node)
    C, expanding_time, reducing_time, result_11, lambda_1, beta_1 = G_qtcs.ALS(alpha, query_node)

    for u in result:  # verified the correctness of the expanding stage
        if u not in C:
            print("wrong!!!!!!!!!!!!!!!!!!!!!!!!!")
...
if __name__ == '__main__':
    ...
    dataset_dir = "D:/Cohesion_Evaluation/Input_Datasets/ALS_Dataset/"
    query_node_dir = "D:/Cohesion_Evaluation/Original_Datasets/Query_nodes/"
    result_dir = "D:/Cohesion_Evaluation/Algorithm_Output/ALS_Results/"
